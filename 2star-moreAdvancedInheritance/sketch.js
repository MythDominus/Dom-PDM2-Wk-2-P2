...
function setup() {
    ...
    animalCalls.push(brian, pete, pedro, whiskers, rex);

    // TEST PARROT
    const polly = new Parrot('Polly', 70, 3, 'hooked');
    console.log(polly.call());         // "Squawk!"
    polly.mimic(animalCalls[Math.floor(Math.random() * animalCalls.length)]);             // logs "Parrot mimics: meow"
    console.log(polly.call());         // "meow"
}
...
class Parrot extends Bird {
    ...
    call() {
        return this.#mimickedPhrase ?? 'Squawk!';
    }
    mimic(animal) {
        if (!animal || typeof animal.call !== 'function') {
            return null;
        }
        const phrase = animal.call(); 
        this.#mimickedPhrase = phrase;
        animalCalls.push(phrase);
        console.log(`Parrot mimics: ${phrase}`);
        return phrase;
    }
}
